...
MODEL_CONFIG="/app/configs/gpt-oss-120b.jsonnet"
...
for TASK in "${TASKS[@]}"; do
  ...
  if [[ " aio jcomqa jemhopqa jsquad niilcqa " =~ " ${TASK} " ]]; then
    EVAL_SETUP="examples/gpt-oss-evaluation/eval_setups/${TASK}.jsonnet"
  else
    EVAL_SETUP="examples/sarashina_evaluation/eval_setups/${TASK}.jsonnet"
  fi
  
  SAVE_DIR="data/results/${TASK}/gpt-oss-120b-16384"
